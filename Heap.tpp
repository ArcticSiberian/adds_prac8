...
void Heap<T>::insert(T value) {
  ...
  //get index of child
  int currentChild = values.size()-1;

  //loop to reoragnise the heap if the child is in the wrong place
  while(currentChild > 0){
    int currentParent = floor((currentChild - 1)/2);

    //swap child and parent
    if(values[currentParent] > values[currentChild]){
      int holding = values[currentParent];
      values[currentParent] = values[currentChild];
      values[currentChild] = holding;
    }

    //assign the parent to be the child for the next loop
...
void Heap<T>::remove(T value) {
  ...
  for(int i = 0; i < values.size(); i++){
    ...
    //break the loop if you find the element
    if(value == values[i]){
      break;
    }
  ...
  //check to see if the value is in the heap
  if(index == values.size()-1 && values[index] != value){
    return;
  }
